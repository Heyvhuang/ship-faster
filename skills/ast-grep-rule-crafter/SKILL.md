...
Project-level scanning requires a `sgconfig.yml` configuration file:

```yaml
# sgconfig.yml (project root directory)
ruleDirs:
...
Check only without fixing, for CI/editor hints:
...
- [ ] 3. Validate: ast-grep scan -r rule.yml src/
- [ ] 4. If false positives → add constraints → re-validate
```

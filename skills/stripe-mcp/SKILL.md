---
name: stripe-mcp
description: Stripe MCP transaction operations skill. Execute transaction operations via Stripe MCP server (https://mcp.stripe.com): customer management, products/prices, invoices, payment links, subscriptions, refunds, dispute handling, balance queries. Triggers when user requests Stripe-related operations including: create customer, create product, create invoice, generate payment link, query transactions, process refunds, manage subscriptions, view disputes, query balance, etc.
---
...
When integrating billing operations into multi-step workflows, all context and artifacts are persisted to disk, passing only paths between agents/sub-agents.

Recommended directory structure (within project): `.claude/runs/<workflow>/<run_id>/`

- Input: `01-input/goal.md` (requirements), `01-input/context.json` (known customer/invoice/subscription/payment_intent IDs)
- Plan: `03-plans/stripe-actions.md` (list of actions to execute; money/contract operations must be written here first and await confirmation)
- Output: `05-final/receipt.md` + `05-final/receipt.json` (object type + ID + key fields + next steps)
- Logs: `logs/events.jsonl` (summary of each tool call; do not log sensitive data verbatim)
...
**Authentication**: Prefer OAuth; if API key is needed, use a restricted key as Bearer token.

**Mode**: Default is test mode. Switching to live requires user to explicitly say "live" with secondary confirmation.

## Three Hard Rules

1. **Query Before Write** - Before creating customer/product/price, use `list_*` or `search_stripe_resources` to check if it already exists, avoiding duplicate objects
2. **Money and Contracts Require Confirmation** - `create_refund`, `cancel_subscription`, `update_subscription`, `update_dispute` must display content and obtain explicit user confirmation before execution
3. **Search When Uncertain** - When unsure about object ID, field, or approach, call `search_stripe_documentation` or `search_stripe_resources` first, do not guess parameters

## Available Tools

| Category | Tool | Description |
|----------|------|-------------|
| Account | `get_stripe_account_info` | Get account information |
...
**Cannot Do** (not in tool table):
- ❌ Create PaymentIntent / direct charge
...
2. **Explain Impact** - Refund amount/cancellation timing/changes to be made
...
- **Currency**: Prefer user-specified currency; if not specified, prefer existing object currency (Price/Invoice/PaymentIntent); if still unclear, ask
- **Amount**: Accept decimal input, automatically convert to smallest unit integer (e.g., £19.99 → 1999)
- **Output Receipt**: Object type + ID + key fields + next step suggestions
...
3. Return cus_xxx + key information
```
...
4. After user confirms, create_refund(payment_intent=pi_xxx, amount if partial refund, reason)
...
3. Ask: cancel immediately or cancel at period end (cancel_at_period_end)
4. After user confirms, cancel_subscription(subscription=sub_xxx)
...
- **Object Not Found**: Use search_stripe_resources or fetch_stripe_resources to find correct ID

...
This document contains parameter descriptions and usage examples for each tool.
...
Get information about the currently connected Stripe account.
...
**Returns**: `cus_xxx` + full customer object
...
| days_until_due | integer | No | Days until due (used with send_invoice) |
...
| invoice | string | No | Invoice ID (if not provided, adds to next invoice) |
...
### finalize_invoice
Finalize an invoice (lock and generate number).
...
**Returns**: Contains `hosted_invoice_url` (customer payment link)
...
| after_completion | object | No | Behavior after completion |
| metadata | object | No | Custom key-value pairs |
...
Create a refund. **Must confirm before executing**.

**Parameters**:
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| payment_intent | string | Yes* | Payment intent ID |
| charge | string | Yes* | Or charge ID (one required) |
| amount | integer | No | Partial refund amount (full if not provided) |
| reason | string | No | duplicate, fraudulent, requested_by_customer |
| metadata | object | No | Custom key-value pairs |

**Confirmation flow**:
1. Show payment_intent/charge ID
2. Show refund amount (full or partial)
3. Show reason
...
Update dispute (submit evidence). **Must confirm before executing**.
...
| metadata | object | No | Custom key-value pairs |
| submit | boolean | No | Whether to submit (cannot be undone after submission) |
...
Cancel subscription. **Must confirm before executing**.
...
**Confirmation flow**:
1. Show subscription ID and current status
...
| duration_in_months | integer | No | Required if duration=repeating |
| max_redemptions | integer | No | Maximum redemption count |
| redeem_by | timestamp | No | Expiration time |

**Example - 20% off, 3 months**:
```json
{
  "percent_off": 20,
  "duration": "repeating",
...
**Use cases**: When unsure about parameter format, best practices, or error handling

...
| entitlements | jsonb | Entitlements configuration |
...
| user_id | string (FK) | Related user → user_profiles.id |
...
|-------|------|-------------|
| id | string (PK) | Run ID |
| project_id | string (FK) | Related project → projects.id |
| batch_id | string (FK) | Related batch → batches.id |
| source_asset_id | string (FK) | Source asset → assets.id |
| recipe | jsonb | Recipe configuration |
...
| run_id | string (FK) | Related run → runs.id |
| asset_id | string (FK) | Related asset → assets.id |
...
### Project Run Status
```sql
-- Projects with their run statistics
SELECT
  p.id, p.name,
...
```sql
-- Recently failed runs (with error info)
